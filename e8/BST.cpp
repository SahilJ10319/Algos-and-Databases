...
{	
	...
	TreeNode* next = root;
	TreeNode* temp = root;
	TreeNode* originalRoot = bst.root; // copy root of bst
	
	helperMethod(originalRoot, root);
}

// Overloaded assignment operator
BST& BST::operator=(const BST& bst)
{
	if (this != &bst)	// prevents the calling object from copying the parameter if they are the same
	{
		clear();
		helperMethod(bst.root, root);
	}
	return *this;
}

// Helper method (if any)
void BST::helperMethod(TreeNode* node, TreeNode*& root)
{
	if (node == nullptr) {
		return;
	}
		...
		root = newNode;
		helperMethod(node->left, root->left);
		helperMethod(node->right, root->right);
	}
}
